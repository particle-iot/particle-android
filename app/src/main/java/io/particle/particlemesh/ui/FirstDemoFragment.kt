...
private val deviceConnections = mutableMapOf<BTDeviceAddress, UIRequestSender>()
...
//                log.info { "Kotlin example completed successfully!" }
    ...
    private fun selectedSender(): UIRequestSender? {
    ...
    private fun connect(btAddress: BTDeviceAddress) {
        launch(UI) {
            ...
            val requestSender = rsFactory.buildRequestSender(btAddress)
    ...
    private fun echoTest() {
        launch(UI) {
            ...
            val requestSender = rsFactory.buildRequestSender(ARGON_ADDY)
            log.info { "sender built" }
